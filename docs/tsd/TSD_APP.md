...
Technical architecture at the app level. For shell implementation detail, see `TSD_BASE_LAYOUT.md`. For Configuration blueprint detail (table registry, routes, MongoDB schemas, data flow), see `TSD_CONFIGURATION.md`.
...
| Database | MongoDB (`kitting_station_v2`) — **connected for Current Kits Configuration and Table Settings**; History does not use it yet |
...
**If a future blueprint (e.g. Live Kitting Activities, History) needs to vary per table, follow the same conventions**: `table_id` as a URL path parameter, a table-registry lookup + `built` guard before rendering, `table_id` stored on any Mongo documents it writes, and (if it needs file storage) a `data/<concern>/table_<id>/` directory.
...
- `app/config/loader.py`'s `_validate_settings()` may not yet include `configuration.tables` in its required-paths fail-fast check — confirm and add if the client wants a hard failure on a missing/malformed table registry rather than a runtime `KeyError`.
- Tables 2 and 3 are registry-only — no Current Kits Configuration, PQPR Analytics, or Table Settings functionality exists for them yet. Building it out is a matter of repeating the Table 1 pattern (routes, templates, data-layer functions already all take `table_id` as a parameter) and flipping `built: true` in `config.yaml`.
